...
fn fs(@builtin(position) fragCoord: vec4<f32>) -> @location(0) vec4<f32> {
  ...
  let colorMix = (sin(uv.x * 1.5 + t * 0.2) * 0.5 + 0.5) * 0.5 + aurora * 0.5;
  var auroraColor = mix(green, cyan, colorMix);
  auroraColor = mix(auroraColor, purple, (sin(t * 0.5 + uv.x) * 0.5 + 0.5) * 0.3);
  
  // add sparkle
  let sparkle = pow(noise(uv * 30.0 + vec2<f32>(t * 2.0, -t * 1.5)), 2.0) * U.sparkliness;
  auroraColor += vec3<f32>(sparkle * 0.5, sparkle * 0.7, sparkle);
  ...
  // add northern lights
  skyColor += auroraColor * aurora * 1.5;
